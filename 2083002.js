...
function action(mode, type, selection) { // calls what you set above in function start, almost all actions are done here 
    ...
    if (status == 0) { // if mode was 1, status would move from -1 to 0. If status is 0, these actions will happen 
        cm.sendSimple("Hello! Are you done fighting Chaos Horntail? If you leave you can't come back to continue.. \r\n #L0# Yes i want to get out of here. #l \r\n #L1# No thank you, i'll continue fighting! #l"); // Opens a window with 2 choices (selections)
    } else if (status == 1) { // NPC advances to next status if a selection is chosen. 
        if (selection == 0) { // selection 0 is #L0#, "Fight him!" 
				cm.warp(100000000,0);
                cm.sendOk("There you go, come back when you are ready!"); // get out of here
                cm.dispose(); 
            }
        } else if (selection == 1) { // continue fighting
                cm.dispose(); 
        } 
    } 
